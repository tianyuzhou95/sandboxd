...
func TestKataBundlePathRejectsTraversal(t *testing.T) {
	handler := &Handler{sandboxRoot: t.TempDir()}
...
func TestKataSandboxStatus(t *testing.T) {
	tests := []struct {
		name string
		in   tasktypes.Status
		want runtimecore.SandboxStatus
	}{
		{name: "created", in: tasktypes.Status_CREATED, want: runtimecore.SandboxStatusCreated},
		{name: "running", in: tasktypes.Status_RUNNING, want: runtimecore.SandboxStatusRunning},
		{name: "stopped", in: tasktypes.Status_STOPPED, want: runtimecore.SandboxStatusExited},
		{name: "paused", in: tasktypes.Status_PAUSED, want: runtimecore.SandboxStatusUnknown},
	}
...
func TestPrepareKataMountsExposesEROFSFilesThroughVirtiofs(t *testing.T) {
	...
	original := runtimecore.StartConfig{
		Rootfs: t.TempDir(),
...
func TestPrepareKataMountsKeepsDirectoriesOnVirtiofs(t *testing.T) {
	...
	kataConfig := runtimecore.StartConfig{
		Mounts: []*runtime.Mount{{
			Type:    "erofs",
			Target:  "/runtime",
			Options: []string{"ro", "loop"},
...
func TestRemoveKataNetworkNamespace(t *testing.T) {
	bundlePath := t.TempDir()
	spec := newKataTestSpec(nil)
	spec.Linux.Namespaces = append(spec.Linux.Namespaces, runtimecore.LinuxNamespace{Type: runtimecore.NetworkNamespace})
	if err := writeKataSpec(filepath.Join(bundlePath, "config.json"), spec); err != nil {
		t.Fatal(err)
	}
...
func TestPrepareKataDirectoryRootfsSeedsMountTargets(t *testing.T) {
	...
	_, err := prepareKataDirectoryRootfs(bundlePath, lowerDir, []runtimecore.Mount{{
		Type:        "bind",
		Source:      fileSource,
...
func newKataTestSpec(resource *runtime.LinuxSandboxResources) *runtimecore.Spec {
	spec := &runtimecore.Spec{
		Annotations: map[string]string{},
		Linux: &runtimecore.Linux{
			Resources: &runtimecore.LinuxResources{
				CPU:    &runtimecore.LinuxCPU{},
				Memory: &runtimecore.LinuxMemory{},
			},
		},
	}
	if resource == nil {
...
func loadKataTestSpec(t *testing.T, bundlePath string) *runtimecore.Spec {
	t.Helper()
	data, err := os.ReadFile(filepath.Join(bundlePath, "config.json"))
	if err != nil {
		t.Fatal(err)
